...
/**
 * Executes a data mining job with only one sequental classification task.
 *
 *
...
class ClassSeqExec extends TraitExec {
  ...
  /**
   * Exection times of each iteration of the cross-validation
   */
  ...
  protected def saveResults(resultSaver: ResultSaver): Unit = {
    ...
    // Number of folds of the execution
    val numFolds = classificationResults.size.toDouble

    // Mean accuracy of the total execution
    val meanAccuracy =
      classificationResults.reduceLeft { _ + _ } / numFolds

    // Mean execution time of the total execution
    val meanExecTime =
